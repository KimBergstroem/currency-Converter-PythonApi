...
| As a returning visitor, I want to check the currency code for a specific country without having to navigate through the entire travel guide. | The application provides a dedicated section where returning visitors can quickly access and view the currency code for a specific country. | <details><summary>Screenshot of result</summary>![Result](docs/testing/user-stories/readme-testing-county-library.png)</details> |
...
| Type   | Bug                                                                                                                                                                                                                                                                                                                                           | Solution                                                                                                                                                                                                                                                                                                                                                                    |
|--------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| run.py | Couldn't find countries in the worksheet that had blank spaces in their names, such as "Sri Lanka" or "New Zealand." This caused the application to display an error message stating that the country does not exist in the list, even though it actually does. | I used `.capitalize()` `.strip()` for remove leading/trailing spaces and also changed in the worksheet from example "Sri Lanka" to "Sri lanka" with only big first letter | 
| run.py | The application crashed when users attempted multiple searches for country or currency due to the "Read requests per minute per user" limit on the Google Sheets API. This `gspread.APIError` occurred when the limit was exceeded. | To address this issue, I implemented a `try-except` block to handle the API error. When the error occurs, an appropriate error message is displayed to the user, advising them to wait or return to the main menu and wait for 1 minute before making another request. |  
| run.py | Endless loop, the currency conversion process was stuck in an endless loop, causing the program to hang indefinitely. | Identified that a while loop condition was missing an exit condition and added a break statement to terminate the loop once the conversion was complete. |
